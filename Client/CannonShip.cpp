...
HRESULT	CCannonShip::Initialize(void)
{
	for(int i = 0; i < 9; ++i)
		STDTex[i] = CTextureMgr::GetInstance()->GetTexture(L"CannonShip", L"STD", i);
	...
	pProfilTex = CTextureMgr::GetInstance()->GetTexture(L"UnitIcon", L"UnitIcon", 40);
	...
	if(m_tInfo.bDeath == false)
	{
		if(m_tInfo.m_ePlayerType == PT_PLAYER)
		{
			...
			if(pTarget)
			{
				...
				if(m_dwTime + 3000 < GetTickCount())
				{
					...
					CObjMgr::GetInstance()->AddObject(L"Cannon", SORT_NORMAL, CFactory<CCannon>::CreateObject(this, pTarget, m_tInfo.m_eSixteenDirType));
				}
			}
		}
		...
		else if(m_tInfo.m_ePlayerType == PT_ENEMY)
		{
			...
			if(pTarget)
			{
				...
				SixteenDirCheckFunc(m_tInfo.vDir,  pTarget->GetInfo()->vPos);
				if(m_dwEnemyTime + 3000 < GetTickCount())
				{
