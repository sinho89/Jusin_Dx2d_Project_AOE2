...
CFire::CFire(CObj*	pUnit, CObj*	pTargetUnit, eSIXTEEN_DIRECTION_TYPE	_eType)
{
	m_pUnit = pUnit;
	...
	m_vLastPos.x = pTargetUnit->GetInfo()->vPos.x + rand() % 50 - 25;
	m_vLastPos.y = pTargetUnit->GetInfo()->vPos.y + rand() % 50 - 25;


	CSoundMgr::GetInstance()->FireStartSound(L"FireShipAttack.wav");
}
...
{
	m_TexArrow = CTextureMgr::GetInstance()->GetTexture(L"Stone_Fire", L"Stone_Fire", 0);
...
int CFire::Update( void )
{
	...
	if(m_fTime >= 1)
	{
		...
		CObjMgr::GetInstance()->AddObject(L"FireBoom", SORT_NORMAL, CFactory<CFireEffect>::CreateObject(m_vLastPos));
		m_tInfo.bAlive = false;
