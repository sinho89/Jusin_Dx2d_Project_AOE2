#pragma once
#include "obj.h"

class CMill :
	public CObj
{
private:
	const TEXINFO*	pMillTex[43];
	const TEXINFO*	pMillTex2[8];
	const TEXINFO*	pMillTex3[10];
	const TEXINFO*	pSelectionLine;
	int m_iTime;
public:
	virtual HRESULT	Initialize(void);
	virtual int		Update(void);
	virtual void	Render(void);
	virtual void	Release(void);
public:
	CMill(void);
	CMill(D3DXVECTOR3 _vPos, eBuildType _Type, ePlayerType _TeamType, BYTE _byDrawID)
	{
		m_tInfo.vPos			= _vPos;
		m_tInfo.m_eBuildType	= _Type;
		m_tInfo.m_ePlayerType	= _TeamType;
		m_tInfo.byDrawID		= _byDrawID;
		m_tInfo.m_eObjType		= OT_BUILD;
		m_tInfo.m_eSaveType		= SRT_FOOD;
		m_tInfo.m_eBuildType	= BT_MILL;
		m_tInfo.m_eSizeType		= BST_MAX;
		//m_tTile.vPos = rTile.vPos;

	};
	virtual ~CMill(void);
};
